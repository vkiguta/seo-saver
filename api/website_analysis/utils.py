...
def get_meta_robots_info(soup):
    ...
    if meta_robots:
        return [{
            "status": "Invalid",
            "message": "The page has a meta attribute({}) with a specific non-indexing of it in search engines.".format(meta)
        }]
    else:
        return [{
            "status": "Valid",
...
def descriptions_list_analysis(keyword, descriptions):
    ...
    if len(descriptions) < 1:
        ...
        results = [{
            "status": "Invalid",
            "message": "The amount of description tag on the page is too large."
        }]
...
def external_links_analysis(external_links):
    ...
    if len(external_links) is None:
        ...
        results = [{
            "status": "Valid",
            "message": "The correct number of external links on the page."
        }]
